...
var haveGPS = false,wunit,utcdiff,favotimeout,strData,choosedDay=0,twidth,maxGraph,minGraph,compGraph,fullGraph;
...
(function(){
  ...
  $("#"+wiset).prop("checked", true);
  $(".wunitbox").change(function(){
    wiset=$('input[name=wunit]:checked').attr('id');
    $.jStorage.set("ewnwunit", wiset);
    $.jStorage.setTTL("ewnwunit", 32000000000);
    $.cookie("ewnwunit", wiset, { expires : 365 });
    wunit=getWunit(wiset);
    gettopbox(strData.data);
    getDaily(strData.daily);
    if($('.frcnavs[data-id="plain"]').hasClass("selected")){getPlain(strData.plaintext);}
    if($('.frcnavs[data-id="table"]').hasClass("selected")){getTable(choosedDay);}
    if($('.frcnavs[data-id="graph"]').hasClass("selected")){getGraph(choosedDay);}
  });
  
  if($.jStorage.get("ewnpunit")){
    piset=$.jStorage.get("ewnpunit");
    $.cookie("ewnpunit", piset, { expires : 365 });
  }
  $("#"+piset).prop("checked", true);
  $(".punitbox").change(function(){
    piset=$('input[name=punit]:checked').attr('id');
    $.jStorage.set("ewnpunit", piset);
    $.jStorage.setTTL("ewnpunit", 32000000000);
    $.cookie("ewnpunit", piset, { expires : 365 });
    gettopbox(strData.data);
    getDaily(strData.daily);
    if($('.frcnavs[data-id="plain"]').hasClass("selected")){getPlain(strData.plaintext);}
  ...
  if($.jStorage.get("ewntunit")){
    tiset=$.jStorage.get("ewntunit");
    $.cookie("ewntunit", tiset, { expires : 365 });
  }
  $("#t"+tiset).prop("checked", true);
  $(".tunitbox").change(function(){
    tiset=$('input[name=tunit]:checked').attr('id').slice(-1);
    $.jStorage.set("ewntunit", tiset);
    $.jStorage.setTTL("ewntunit", 32000000000);
    $.cookie("ewntunit", tiset, { expires : 365 });
    gettopbox(strData.data);
    getDaily(strData.daily);
    if($('.frcnavs[data-id="plain"]').hasClass("selected")){getPlain(strData.plaintext);}
  ...
  $(".frcnavs").on("click", function() {
    $("#plaintable, #frctable, #graphtable, #mapwrapper, #comptable").hide();
    $('.frcnavs').removeClass("selected");
    $(this).addClass("selected");
    fg=$(this).attr("data-id");
    if(fg=="plain"){$("#plaintable").show();getPlain(strData.plaintext);}
    if(fg=="table"){$("#frctable").show();getTable(choosedDay);}
    if(fg=="graph"){$("#graphtable").show();getGraph(choosedDay);}
  });
  
  $("#wunitlink").on("click", function() {
    if($('#wunitbar').is(':visible')){
      $("#wunitbar").hide();
    }else{
      $("#wunitbar").show();
    }
...
function getData(){
  ...
  wget([dpath], function(points) {
    ...
    gettopbox(strData.data);
    getDaily(strData.daily);
    getPlain(strData.plaintext);
  });
}

function gettopbox(data){
  $("#frctopclock span").html(data[0].time);
  $("#frctopicon").html('<svg class="svg-ic svg-ictop" viewBox="0 0 512 512"><g>'+geticon(data[0].icon)+'</g></svg>');
  $("#frctoptemp span").html(Math.round(fixTemp(data[0].temp))+'&deg;').css({color:data[0].tempRoundedColor});
  $("#frctopwind").html('<i class="wi wi-wind from-'+data[0].wdir+'-deg" style="font-size:20px;top:-4px;"></i><span id="topwind">'+Math.round(fixWspd(data[0].wspd))+' '+wunit+'</span>');
  $("#toprain").html(fixPrec(data[0].prec,1)+' '+piset);
  $("#toppop").html(fixPop(data[0].pop)+'%');
  $("#topts").html(fixTs(data[0].tsrisk)+'%');
  $("#topsnow").html(fixSnowf(data[0].snowfall)+' '+snowset);
  if(data[0].temp<5){$("#frctoplight").hide();}else{$("#frctopsnow").hide();}
}
...
function getPlain(data){
  ...
  $.each(data, function(i, item) {
    if(b%2!=0){odd=" odd";}else{odd="";}
    dco='<div class="frctblrow '+odd+'">'+
        '<div class="tbltime" style="text-align:left;padding-left:6px;">'+item.time+'</div>'+
        '<div class="tblic"><svg class="svg-ic svg-ictbl" viewBox="0 0 512 512"><g>'+geticon(item.icon)+'</g></svg></div>'+
        '<div class="tbltemp" style="color:'+item.tempRoundedColor+'">'+Math.round(fixTemp(item.temp))+'&deg;</div>'+
        '<div class="tblfeel">'+getwindicon_svg(item.wspd,item.wdir,undefined,item.bft,wunit)+'</div>'+
        '<div class="tblfeel">'+fixPrec(item.prec,0)+' '+piset+'</div>'+
        '<div class="tblfeel ppophead">';
      if(item.pop>0 && twidth>750){
        dco+='<i class="wi wi-umbrella" style="font-size:20px;top:0px;position: relative;margin:0 10px;"></i>'+fixPop(item.pop)+'%';
        ppop++;
      }
      dco+='</div><div class="tblfeel puvihead">';
      if(item.uv>0 && twidth>750){
        dco+='<span class="sun-spr sun-uv'+item.uv+'"></span>';
        puvi++;
      }
      dco+='</div><div class="tbltxt">'+item.txt+'</div></div>';
    $("#plaintbl").append(dco);
    b++;
  });
  if(ppop>0){$(".ppophead").show();}else{$(".ppophead").hide();}
  if(puvi>0){$(".puvihead").show();}else{$(".puvihead").hide();}
...
function getDaily(data){
  ...
  $.each(data, function(i, item) {
    ...
    dco='<li class="touchcarousel-item" data-id="'+d+'">'+
      ...
      '<div class="daytemps">'+
        '<div class="daytemp" style="color:'+item.minTempRoundedColor+'">'+Math.round(fixTemp(item.minTemp))+'&deg;</div>'+
        '<div class="daytemp" style="color:'+item.maxTempRoundedColor+'">'+Math.round(fixTemp(item.maxTemp))+'&deg;</div></div>'+
      '<div class="frcdayrest">'+
        '<i class="wi wi-raindrops" style="color:#4F94CD;font-size:30px;top:3px;left:0px"></i><span class="dayrain">'+pre+' '+piset+'</span>'+
  ...
  $('#dailyCarousel .touchcarousel-item[data-id="0"]').addClass("activeday");
  $("#dailyCarousel").touchCarousel({scrollbar:false});
  $("#dailyCarousel .touchcarousel-item").on("click", function() {
    $('#dailyCarousel .touchcarousel-item').removeClass("activeday");
    $(this).addClass("activeday");
    choosedDay=parseInt($(this).attr("data-id"));
    if($('.frcnavs[data-id="table"]').hasClass("selected")){ getTable(choosedDay); }
    if($('.frcnavs[data-id="graph"]').hasClass("selected")){ getGraph(choosedDay); }
  });
}
...
function getTable(a){
  ...
  $.each(strData.data, function(i, item) {
    if(item.day===a){
      if(b==0 && item.time=="23:00"){return true;}
      if(b%2!=0){odd=" odd";}else{odd="";}
      if(piset=="mm"){
      ...
      if(snowset=="cm"){
        if(item.snowfall>0&&item.snowfall<1){snowf=item.snowfall;}else{snowf=Math.round(item.snowfall);}
      }else{
        snowf=item.snowfall;
      }
      if(twidth<=750){
        prectip=' data-toggle="tooltip" data-placement="top" title="<i class=\'wi wi-umbrella\' style=\'font-size:20px;top:0px;position: relative;margin:0 10px 0 0;\'></i>'+fixPop(item.pop)+'% <i class=\'wi wi-lightning\' style=\'color:#FFA500;font-size:20px;top:2px;position: relative;margin:0 10px;\'></i>'+fixTs(item.tsrisk)+'%"';
      }else{prectip="";}
      
      dco='<div class="frctblrow '+odd+'">'+
        '<div class="tbltime">'+item.time+'</div>'+
        '<div class="tblic"><svg class="svg-ic svg-ictbl" viewBox="0 0 512 512"><g>'+geticon(item.icon)+'</g></svg></div>'+
        '<div class="tbltemp" style="color:'+item.tempRoundedColor+'">'+Math.round(fixTemp(item.temp))+'&deg;</div>'+
        '<div class="tblfeel">'+Math.round(fixTemp(item.feels))+'&deg;</div>'+
        '<div class="tblfeel">'+getwindicon_svg(item.wspd,item.wdir,item.wgst,item.bft,wunit)+'</div>'+
        '<div class="tblfeel" data-toggle="tooltip" data-placement="top" title="'+dewtxt+': '+fixTemp(item.dewp)+'&deg;">'+Math.round(item.rh)+'%</div>'+
        '<div class="tblfeel"'+prectip+'>'+pre+' '+piset+'</div>'+
        '<div class="tblfeel pophead">';
      if(item.pop>0 && twidth>750){
        dco+='<i class="wi wi-umbrella" style="font-size:20px;top:0px;position: relative;margin:0 10px;"></i>'+fixPop(item.pop)+'%';
        pop++;
      }
      dco+='</div><div class="tblfeel snowhead">';
      if(snowf>0 && item.temp<=1.8 && twidth>750){
        dco+='<i class="fa fa-snowflake-o" style="color:#ffffff;font-size:20px;top:2px;left:-4px;position:relative"></i>'+fixSnowf(snowf)+' '+snowset;
        snow++;
      }
      dco+='</div><div class="tblfeel tshead">';
      if(item.tsrisk>5 && twidth>750){
        dco+='<i class="wi wi-lightning" style="color:#FFA500;font-size:20px;top:2px;position: relative;margin:0 10px;"></i>'+fixTs(item.tsrisk)+'%';
        ts++;
      }
      dco+='</div><div class="tblfeel uvihead">';
      if(item.uvi>0 && twidth>750){
        dco+='<span class="sun-spr sun-uv'+item.uvi+'"></span>';
        uvi++;
      }
      dco+='</div><div class="tblfeel tblbaro">'+fixBaro(item.prmsl)+' hPa</div>';
      $("#frctbl").append(dco);
  ...
  d=0;
  $.each(strData.daily, function(i, item) {
    if(d===a){
      sm = '<div id="sunmoonday">'+item.day+'</div>';
      if(item.sunrise=="midnightsun"){
        sm+= '<div><i class="wi wi-sunrise sunicon"></i>'+txtmidnightsun+'</div>';
      }else if (item.sunrise=="polarnight"){
        sm+= '<div><i class="wi wi-sunset sunicon"></i>'+txtpolarnight+'</div>';
      }else{
        sm+= '<div><i class="wi wi-sunrise sunicon"></i>'+item.sunrise+'</div>'+
          '<div><i class="wi wi-sunset sunicon"></i>'+item.sunset+'</div>';
      }
      if(twidth>750){
        sm+= '<div><i class="wi wi-moonrise moonicon"></i>'+item.moonrise+'</div>'+
          '<div><i class="wi wi-moonset moonicon"></i>'+item.moonset+'</div>'+
          '<div><i class="wi wi-moon-'+item.moonage+' moonicon"></i></div>';
      }
      $("#sunmoon").html(sm);
      if(latitude<0){$(".wi-moon-"+item.moonage).addClass("wi-rotate-180");}
    }
    d++;
  });
  
  if(pop>0){$(".pophead").show();}else{$(".pophead").hide();}
  if(ts>0){$(".tshead").show();}else{$(".tshead").hide();}
  if(snow>0){$(".snowhead").show();}else{$(".snowhead").hide();}
  if(uvi>0){$(".uvihead").show();}else{$(".uvihead").hide();}
  $('[data-toggle="tooltip"]').tooltip({html:true});
...
function getGraph(a){
  ...
  day1Max = strData.data[0].timestamp+(24*3600);
  day1Min = strData.data[0].timestamp;
  $.each(strData.data, function(i, item) {
    if(a===item.day||(a==0&&item.timestamp>=day1Min&&item.timestamp<=day1Max)||fullGraph){
      tstamp=item.timestamp*1000;
      tempdata.push([tstamp, parseFloat(fixTemp(item.temp))]);
      feeldata.push([tstamp, parseFloat(fixTemp(item.feels))]);
      dewdata.push([tstamp, parseFloat(fixTemp(item.dewp))]);
      precdata.push([tstamp, parseFloat(fixPrec(item.prec,1))]);
      tsdata.push([tstamp, parseFloat(item.tsrisk)]);
      barodata.push([tstamp, parseFloat(fixBaro(item.prmsl))]);
      windtip.push([tstamp, parseFloat(fixWspd(item.wspd))]);
      if(twidth>650){
        svgdata.push({"time":tstamp, "dir":item.wdir, "bft": item.bft, "cond": item.icon});
      }else if(typeof item.prec3h !== 'undefined'){
        svgdata.push({"time":tstamp, "dir":item.wdir, "bft": item.bft, "cond": item.icon});
      }
    }
  });
  getHC();
  if($('#graphbuttons div div[data-id="frc"]').hasClass("active")){
    lastWX=0,lastWind=0;
    chart = new Highcharts.Chart(tempoptions);
    drawgraphiconswx(svgdata, "hourgraph", 20, 0);
    drawgraphiconswind(svgdata,357);
  }else{
    chart = new Highcharts.Chart(compoptions);
  }
  $("#graphbuttons div div").unbind().on("click", function() {
    fgr=$(this).attr("data-id");
    if(fgr!=="fullgraph"){$("#graphbuttons div div").removeClass("active");}
    if(fullGraph){$('#graphbuttons div div[data-id="fullgraph"]').addClass("active");}
    ...
    if(fgr=="frc"){
      ...
      drawgraphiconswind(svgdata,357);
      compGraph=false;
    }else if(fgr=="fullgraph"){
      if(fullGraph){
        fullGraph=false;
        $('#graphbuttons div div[data-id="fullgraph"]').removeClass("active");
        getGraph(choosedDay);
      }else{
        fullGraph=true;
        $('#graphbuttons div div[data-id="fullgraph"]').addClass("active");
        getGraph(null);
      }
      if(compGraph){
        chart = new Highcharts.Chart(compoptions);
...
function fixTemp(val){
  if(tiset=="f"){return ((val*1.8)+32).toFixed(1);}
  else return (val*1).toFixed(1);
}

function fixBaro(val){
  if(baroset=="inHg"){return (val/33.8638866).toFixed(2);}
  else return val;
}

function fixPrec(val,dec){
  if(piset=="in"){return (val/25.4).toFixed(2);}
  else return (val*1).toFixed(dec);
}

function fixSnowf(val){
  if(snowset=="in"){return (val*0.3937).toFixed(2);}
  else{return val;}
}
...
function getHC(){
  ...
  if(piset=="mm"){var pr=5;}else{pr=0.5;}
  tempYAxis = [
    { // Temperature
      ...
      title: {text: null},labels: {enabled: false},gridLineWidth: 0,min:0,minRange:pr,tickLength: 0
    },{ // Air pressure 
  ...
  tempoptions = {
    ...
    series: [{
      ...
      name: prectxt,data: precdata,tooltip: {valueSuffix: " "+piset},color:"#4572A7",type: "column",yAxis: 1,zIndex:2,
